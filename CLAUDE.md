...
There is no migration system. Schema changes happen by editing the `init-sheets` map and re-hitting `GET /api/init-sheets` (with an admin session or `Bearer CRON_SECRET`), which adds missing columns without touching existing data.
