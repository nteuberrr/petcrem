...
-- ⚠️ `by default`, NO `always`: la capa de datos (lib/datastore appendRow) inserta
-- el id EXPLÍCITO que le da getNextId(), y `generated always` lo rechaza con
-- "cannot insert a non-DEFAULT value into column id". Es la convención de todas
-- las tablas del proyecto (ver supabase/schema-principal.sql).
...
-- Repara la tabla si se creó con la versión anterior (`generated always`), que
-- hacía fallar todo insert con "cannot insert a non-DEFAULT value into column id".
alter table public.conciliacion_sii alter column id set generated by default;
