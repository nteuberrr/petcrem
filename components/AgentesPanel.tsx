...
export default function AgentesPanel() {
  ...
  return (
    <div className="space-y-4">
      <div className="inline-flex gap-1 bg-gray-100 border border-gray-300 rounded-lg p-1">
        {([['whatsapp', '💬 WhatsApp'], ['marketing', '🧠 Marketing']] as const).map(([k, label]) => (
          <button key={k} onClick={() => setSub(k)}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${sub === k ? 'bg-brand text-white' : 'text-gray-600 hover:bg-white'}`}>
            {label}
          </button>
        ))}
      </div>
