      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="max-w-2xl">
            <p className="text-sm text-gray-700">
              Los valores legales de cada mes. Es lo que mantiene el módulo al día: los topes imponibles se reajustan
              todos los años y el aporte previsional del empleador sube por tramos hasta 2033.
            </p>
            <p className="mt-1 text-xs text-gray-500">
              La UF y la UTM las publica el Banco Central y el SII cada mes; hay que cargarlas antes de liquidar.
            </p>
          </div>
        ...
        {faltaActual && (
          <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
            ...
            (se pueden duplicar del mes anterior).
          </div>
        )}

        <div className="mt-4 rounded-xl border border-brand/30 bg-cream p-3 text-xs text-gray-700">
          <b className="text-brand">Ojo con agosto 2026:</b> desde las remuneraciones de este mes el aporte previsional
          del empleador pasa de 1% a 3,5% y absorbe el SIS — el 1,38% deja de pagarse por separado. Al duplicar un
          período anterior, revisa esas tasas.
        </div>
      </Card>
    ...
    <Modal open onClose={onCerrar} title={`Parámetros de ${nombrePeriodo(periodo)}`} size="3xl">
      ...
      {f && (
        <div className="space-y-5">
          ...
          <Grupo titulo="Valores del mes">
            <N label="Valor UF" value={f.valor_uf} onChange={v => set('valor_uf', v)} paso="0.01" pie="lo publica el Banco Central" />
            <N label="Valor UTM" value={f.valor_utm} onChange={v => set('valor_utm', v)} pie="lo publica el SII" />
