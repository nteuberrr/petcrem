...
/** Estilo global que se añade SIEMPRE al prompt para forzar fotorrealismo. */
const STYLE_SUFFIX =
  'Photorealistic, natural editorial photography. Real people and real animals, lifelike and authentic, ' +
  'soft natural daylight, shallow depth of field, high detail, calm and warm mood. ' +
  ...
  'Clean, uncluttered composition that works well as an image inside an email.'
...
export interface GenerarImagenOpts {
  /** Descripción de la escena (la "dirección de arte"). */
  prompt: string
...
export async function generarImagen(opts: GenerarImagenOpts): Promise<ImagenGenerada> {
  ...
  const aspectTxt = opts.aspect && ASPECT_HINT[opts.aspect] ? ` Composition: ${ASPECT_HINT[opts.aspect]}.` : ''
