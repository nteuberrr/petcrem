...
- Estética: foto realista y natural, CÁLIDA y con alma — nunca plana, fría ni desaturada. Buscá cercanía y emoción: luz natural cálida y envolvente, pero VARIÁ la hora/mood entre fotos (golden hour, y también día claro y luminoso, mediodía suave o tarde cálida de interior) — que el feed no parezca "la misma sesión". Color rico pero natural, texturas y PROFUNDIDAD (primer plano nítido + fondo desenfocado), un instante cotidiano que cuente una pequeña historia. Atmósfera serena, contenida y esperanzadora — nunca dramática ni lúgubre. Nada de estilo cinematográfico exagerado (humo, contraluces, flares) ni pinta de stock genérico/plantilla.
...
export const ESTILO_MARCA_EN =
  `Brand look (Crematorio Alma Animal — pet cremation): warm, intimate and heartfelt, with rich natural color, warm natural light and gentle depth (soft foreground and background) — inviting and full of life, telling a small everyday story. Never flat, cold, dull, desaturated, dramatic or gloomy. ` +
  `The SCENE must feel warm: warm neutrals (cream ${BRAND.cream}, beige, honey wood, soft natural greens) under warm natural light — VARY the time of day and mood across images (golden hour, but also soft bright daylight or a warm indoor afternoon; don't make every photo the same golden look) — but NEVER a blue or cold color cast over the image. The brand navy (${BRAND.navy}) belongs to graphic design, NOT to photos: do not fill the scene with blue blankets, blue clothing or blue walls (at most one small, subtle navy detail). A little warm gold (${BRAND.amber}) as an accent reads on-brand. Avoid pure black, intense reds, neon/fluorescent colors and clinical "health" greens. ` +
