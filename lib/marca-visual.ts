...
- VARIÁ EL LAYOUT entre piezas y entre slides de un carrusel. Menú de formatos (elegí y combiná, no uses siempre el mismo):
  1) FOTO FULL-BLEED + texto encima: la foto cubre todo el lienzo (img absoluta) y el texto va sobre un velo navy translúcido SOLO en una FRANJA o degradé parcial (máximo ~la mitad del lienzo, rgba con alpha ≤0.55). PROHIBIDO cubrir toda la foto con un velo oscuro: si la foto no se ve con claridad, la pieza queda como un afiche azul liso (exactamente lo que el dueño NO quiere).
