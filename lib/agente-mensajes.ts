...
- EL RECARGO SE COBRA UNA SOLA VEZ (regla dura — caso Yami, 2026-07-28): el recargo fuera de horario es UNO POR ATENCIÓN, aunque caigan fuera de horario las dos partes del servicio. Solo eutanasia fuera de horario → un recargo, del lado de la EUTANASIA; solo el retiro de cremación fuera de horario → un recargo, del lado de la CREMACIÓN; LAS DOS fuera de horario → SIGUE SIENDO UN SOLO recargo (nunca el doble) y lo lleva la EUTANASIA. Y en el mensaje: el recargo se nombra UNA vez, como línea del desglose, con el total ya sumado. JAMÁS des un precio o un total y después escribas "a esto hay que sumarle $…" — eso confunde al cliente y parece que le cobras dos veces. Un solo desglose, un solo total final.
...
export interface AccionCotizarCremacion {
  ...
  hora?: string
  /** Hora (HH:MM) del PROCEDIMIENTO de eutanasia, si esta cremación va con una.
   *  Con ella la herramienta decide sola quién lleva el recargo — el modelo no
   *  tiene que razonarlo (que es donde se equivocaba). */
  hora_eutanasia?: string
  /** Respaldo del flag anterior: true si la eutanasia YA lleva el recargo. */
...
const TOOL_COTIZAR_CREMACION: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      hora_eutanasia: { type: 'string', description: 'Hora HH:MM del PROCEDIMIENTO de eutanasia, si esta cremación va junto a una. Pásala SIEMPRE que haya eutanasia: con ella la herramienta decide sola de qué lado va el recargo por fuera de horario (si la eutanasia es después de las 18:00 lo lleva ella; si no, y el retiro sí se pasa, lo lleva la cremación). No lo decidas tú.' },
    },
    required: ['peso'],
  },
}
