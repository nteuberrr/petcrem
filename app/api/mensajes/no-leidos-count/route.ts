...
import { contarNoLeidos, contarNoLeidosPorCategoria } from '@/lib/mensajes'
...
export async function GET() {
  ...
  try {
    const [count, porCategoria] = await Promise.all([contarNoLeidos(), contarNoLeidosPorCategoria()])
    return NextResponse.json({ count, porCategoria }, { headers: { 'Cache-Control': 'no-store' } })
  } catch (e) {
