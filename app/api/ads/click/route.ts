...
/**
 * Registra un clic de Google Ads que aterrizó en el sitio público y devuelve el
 * código corto que el navegador incrusta en el link de WhatsApp.
 *
...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json().catch(() => null) as
      { gclid?: unknown; gbraid?: unknown; wbraid?: unknown; landing?: unknown } | null
    if (!body) return NextResponse.json({ codigo: null })

    const limpio = (v: unknown): string | null => {
      const s = typeof v === 'string' ? v.trim() : ''
      // Los identificadores de Google son alfanuméricos con - y _.
